...
import { PhysicalEdge, ParsedSpec, ThematicPoint } from "streetweave";
...
export async function loadSegmentData(layerSpec: ParsedSpec) {
  ...
  if (layerSpec.unit.splits !== 1) {
    edges = subdivideEdges(physicalData, thematicData.attributeStats, layerSpec.unit.splits);
  }
  
  const processedEdges = await applySpatialAggregation(edges, thematicData.data, layerSpec);
  return { processedEdges, thematicData };
}

function subdivideEdges(edges: PhysicalEdge[], attributeStats: Record<string, any>, layerSpecSplits: string | number | undefined ) {
